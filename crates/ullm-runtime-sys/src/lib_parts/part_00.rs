...
unsafe extern "C" {
    ...
    fn ullm_runtime_aq4_matvec_batch_wmma_prototype_v2_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
...
/// Directly invokes the experimental gfx1201 rocWMMA AQ4 group16 M=128 v2 kernel.
///
/// This forced A/B path has no dispatch or environment dependency and never falls back. It uses
/// a separate HIPRTC module from the production v1 kernel while retaining the same shape and
/// buffer contract.
#[allow(clippy::too_many_arguments)]
pub fn aq4_matvec_batch_wmma_prototype_v2_f32(
    index_buffer: &RuntimeBuffer,
    scale_buffer: &RuntimeBuffer,
    codebook_buffer: &RuntimeBuffer,
    scale_values_buffer: &RuntimeBuffer,
    ...
    if scale_count == 0 {
        return Err("AQ4 WMMA prototype v2 scale table is empty".to_string());
    }
    if group_size != 16 {
        return Err("AQ4 WMMA prototype v2 requires group size 16".to_string());
    }
    if batch_count != 128 {
        return Err("AQ4 WMMA prototype v2 requires batch count M=128".to_string());
    }
    if rows == 0 || cols == 0 || !rows.is_multiple_of(16) || !cols.is_multiple_of(32) {
        return Err(
            "AQ4 WMMA prototype v2 requires nonzero rows divisible by 16 and cols divisible by 32"
                .to_string(),
        );
    }
    if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
        return Err(
            "AQ4 WMMA prototype v2 tensor scale must be finite and greater than zero".to_string(),
        );
    }
    let grid_x = rows
        .checked_div(16)
        .ok_or_else(|| "AQ4 WMMA prototype v2 row grid is invalid".to_string())?;
    let grid_y = batch_count
        .checked_div(128)
        .ok_or_else(|| "AQ4 WMMA prototype v2 batch grid is invalid".to_string())?;
    if grid_x == 0 || grid_y == 0 || grid_x > u32::MAX as usize || grid_y > u32::MAX as usize {
        return Err("AQ4 WMMA prototype v2 dimensions exceed HIP grid limit".to_string());
    }
    let elements = rows
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 WMMA prototype v2 matrix element count overflows".to_string())?;
    let index_bytes = elements / 2;
    let groups = elements / group_size;
    let scale_value_bytes = scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v2 scale value byte size overflows".to_string())?;
    let input_elements = batch_count
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 WMMA prototype v2 input element count overflows".to_string())?;
    let output_elements = batch_count
        .checked_mul(rows)
        .ok_or_else(|| "AQ4 WMMA prototype v2 output element count overflows".to_string())?;
    let input_bytes = input_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v2 input byte size overflows".to_string())?;
    let output_bytes = output_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v2 output byte size overflows".to_string())?;
    let row_scale_bytes = row_scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v2 row scale byte size overflows".to_string())?;
    check_copy_range(0, index_bytes, index_buffer.size()?)?;
    check_copy_range(0, groups, scale_buffer.size()?)?;
    check_copy_range(0, 16 * std::mem::size_of::<f32>(), codebook_buffer.size()?)?;
    check_copy_range(0, scale_value_bytes, scale_values_buffer.size()?)?;
    ...
    let row_scale_raw = row_scale_buffer
        ...
        .unwrap_or(std::ptr::null_mut());
    status_to_result(unsafe {
        ullm_runtime_aq4_matvec_batch_wmma_prototype_v2_f32(
            index_buffer.raw.as_ptr(),
            scale_buffer.raw.as_ptr(),
            codebook_buffer.raw.as_ptr(),
            scale_values_buffer.raw.as_ptr(),
