...
unsafe extern "C" {
    ...
    fn ullm_runtime_linear_attn_recurrent_shuffle_prototype_f32(
        q_buffer: *const RawRuntimeBuffer,
        k_buffer: *const RawRuntimeBuffer,
        v_buffer: *const RawRuntimeBuffer,
        gate_buffer: *const RawRuntimeBuffer,
...
/// Directly invokes the experimental gfx1201 wave-shuffle recurrent kernel for the exact Qwen3.5
/// Gated DeltaNet geometry. It is intentionally separate from production dispatch and has no
/// fallback, so callers can use it for controlled GPU differential and timing runs.
#[allow(clippy::too_many_arguments)]
pub fn linear_attn_recurrent_shuffle_prototype_f32(
    q: &RuntimeBuffer,
    k: &RuntimeBuffer,
    v: &RuntimeBuffer,
    gate: &RuntimeBuffer,
    ...
    if key_heads != 16 || value_heads != 32 || key_dim != 128 || value_dim != 128 {
        return Err(
            "linear attention recurrent shuffle prototype requires Qwen3.5 key/value heads=16/32 and key/value dim=128"
                .to_string(),
        );
    }
    if sequence_len == 0 {
        return Err(
            "linear attention recurrent shuffle prototype sequence_len must be greater than zero"
                .to_string(),
        );
    }

    let key_head_sequence_elements = key_heads.checked_mul(sequence_len).ok_or_else(|| {
        "linear attention recurrent shuffle prototype key head-sequence element count overflows"
            .to_string()
    })?;
    let value_head_sequence_elements = value_heads.checked_mul(sequence_len).ok_or_else(|| {
        "linear attention recurrent shuffle prototype value head-sequence element count overflows"
            .to_string()
    })?;
    let q_elements = key_head_sequence_elements
        .checked_mul(key_dim)
        .ok_or_else(|| {
            "linear attention recurrent shuffle prototype q element count overflows".to_string()
        })?;
    let v_elements = value_head_sequence_elements
        .checked_mul(value_dim)
        .ok_or_else(|| {
            "linear attention recurrent shuffle prototype v/output element count overflows"
                .to_string()
        })?;
    let state_elements = value_heads
        .checked_mul(key_dim)
        .ok_or_else(|| {
            "linear attention recurrent shuffle prototype state element count overflows"
                .to_string()
        })?
        .checked_mul(value_dim)
        .ok_or_else(|| {
    ...
    let q_bytes = q_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| {
            "linear attention recurrent shuffle prototype q byte size overflows".to_string()
        })?;
    let k_bytes = q_bytes;
    let v_output_bytes = v_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| {
            "linear attention recurrent shuffle prototype v/output byte size overflows"
                .to_string()
        })?;
    let gate_beta_bytes = value_head_sequence_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| {
            "linear attention recurrent shuffle prototype gate/beta byte size overflows"
                .to_string()
        })?;
    let state_bytes = state_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| {
            "linear attention recurrent shuffle prototype state byte size overflows".to_string()
        })?;

    check_linear_attention_recurrent_copy_range("q", 0, q_bytes, q.size()?)?;
    check_linear_attention_recurrent_copy_range("k", 0, k_bytes, k.size()?)?;
    check_linear_attention_recurrent_copy_range("v", 0, v_output_bytes, v.size()?)?;
    check_linear_attention_recurrent_copy_range(
        "gate",
        0,
        gate_beta_bytes,
        gate.size()?,
    ...
    let stream = stream.map_or(std::ptr::null_mut(), |stream| stream.raw.as_ptr());
    status_to_result(unsafe {
        ullm_runtime_linear_attn_recurrent_shuffle_prototype_f32(
            q.raw.as_ptr(),
            k.raw.as_ptr(),
            v.raw.as_ptr(),
            gate.raw.as_ptr(),
