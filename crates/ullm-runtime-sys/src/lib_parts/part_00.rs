...
unsafe extern "C" {
    ...
    fn ullm_runtime_aq4_matvec_wide_load_prototype_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
    ...
    fn ullm_runtime_aq4_matvec_batch_register_bm8_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
    ...
    fn ullm_runtime_aq4_matvec_batch_wmma_ragged_m_prototype_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
    ...
    fn ullm_runtime_aq4_matvec_batch_wmma_prototype_v3_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
    ...
    fn ullm_runtime_aq4_matvec_batch_wmma_group8_prototype_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
    ...
    fn ullm_runtime_aq4_matvec_add_wide_load_prototype_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
    ...
    fn ullm_runtime_aq4_matvec_add_shuffle_prototype_f32(
        index_buffer: *const RawRuntimeBuffer,
        scale_buffer: *const RawRuntimeBuffer,
        codebook_buffer: *const RawRuntimeBuffer,
        scale_values_buffer: *const RawRuntimeBuffer,
...
/// Direct-only gfx1201 M=1 wide-load AQ4 prototype. It accepts group8/group16 matrices whose
/// columns are divisible by 32, performs no fallback, and never changes production dispatch.
#[allow(clippy::too_many_arguments)]
pub fn aq4_matvec_wide_load_prototype_f32(
    index_buffer: &RuntimeBuffer,
    scale_buffer: &RuntimeBuffer,
    codebook_buffer: &RuntimeBuffer,
    scale_values_buffer: &RuntimeBuffer,
...
/// Directly invokes the cached gfx1201/group16 register BM8 kernel.
///
...
#[allow(clippy::too_many_arguments)]
pub fn aq4_matvec_batch_register_bm8_f32(
    index_buffer: &RuntimeBuffer,
    scale_buffer: &RuntimeBuffer,
    codebook_buffer: &RuntimeBuffer,
    scale_values_buffer: &RuntimeBuffer,
    ...
    if scale_count == 0 {
        return Err("AQ4 register BM8 batch scale table is empty".to_string());
    }
    if group_size != 16 {
        return Err("AQ4 register BM8 batch requires group size 16".to_string());
    }
    if rows == 0 || cols == 0 || batch_count < 8 {
        return Err(
            "AQ4 register BM8 batch requires nonzero rows/cols and batch count at least 8"
                .to_string(),
        );
    }
    if !rows.is_multiple_of(32) || !cols.is_multiple_of(128) {
        return Err(
            "AQ4 register BM8 batch requires rows divisible by 32 and cols divisible by 128"
                .to_string(),
        );
    }
    if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
        return Err(
            "AQ4 register BM8 batch tensor scale must be finite and greater than zero".to_string(),
        );
    }
    let grid_x = rows
        .checked_add(31)
        .map(|value| value / 32)
        .ok_or_else(|| "AQ4 register BM8 batch row grid overflows".to_string())?;
    cols.checked_add(127)
        .ok_or_else(|| "AQ4 register BM8 batch column geometry overflows".to_string())?;
    let grid_y = batch_count
        .checked_add(7)
        .map(|value| value / 8)
        .ok_or_else(|| "AQ4 register BM8 batch grid overflows".to_string())?;
    if grid_x > u32::MAX as usize || grid_y > u32::MAX as usize {
        return Err("AQ4 register BM8 batch dimensions exceed HIP grid limit".to_string());
    }
    let elements = rows
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 register BM8 batch matrix element count overflows".to_string())?;
    let index_bytes = elements / 2;
    let groups = elements / group_size;
    let scale_value_bytes = scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 register BM8 batch scale value byte size overflows".to_string())?;
    let input_elements = batch_count
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 register BM8 batch input element count overflows".to_string())?;
    let output_elements = batch_count
        .checked_mul(rows)
        .ok_or_else(|| "AQ4 register BM8 batch output element count overflows".to_string())?;
    let input_bytes = input_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 register BM8 batch input byte size overflows".to_string())?;
    let output_bytes = output_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 register BM8 batch output byte size overflows".to_string())?;
    let row_scale_bytes = row_scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 register BM8 batch row scale byte size overflows".to_string())?;
    check_copy_range(0, index_bytes, index_buffer.size()?)?;
    check_copy_range(0, groups, scale_buffer.size()?)?;
    check_copy_range(0, 16 * std::mem::size_of::<f32>(), codebook_buffer.size()?)?;
    check_copy_range(0, scale_value_bytes, scale_values_buffer.size()?)?;
    ...
    let row_scale_raw = row_scale_buffer
        ...
        .unwrap_or(std::ptr::null_mut());
    status_to_result(unsafe {
        ullm_runtime_aq4_matvec_batch_register_bm8_f32(
            index_buffer.raw.as_ptr(),
            scale_buffer.raw.as_ptr(),
            codebook_buffer.raw.as_ptr(),
            scale_values_buffer.raw.as_ptr(),
...
/// Directly invokes the production double-buffered gfx1201 rocWMMA AQ4 group16 M=128 kernel.
///
...
/// Directly invokes the isolated gfx1201 group16 WMMA ragged-M experiment.
///
/// One physical M=128 CTA is launched, but `m_actual` (1..=128) is passed to the kernel and
/// bounds every activation-vector load and every final output store.  Callers may therefore use
/// input and output buffers sized exactly for `m_actual`, with no 128-row padding.
#[allow(clippy::too_many_arguments)]
pub fn aq4_matvec_batch_wmma_ragged_m_prototype_f32(
    index_buffer: &RuntimeBuffer,
    scale_buffer: &RuntimeBuffer,
    codebook_buffer: &RuntimeBuffer,
    scale_values_buffer: &RuntimeBuffer,
    ...
    if scale_count == 0 {
        return Err("AQ4 WMMA ragged-M prototype scale table is empty".to_string());
    }
    if group_size != 16 {
        return Err("AQ4 WMMA ragged-M prototype requires group size 16".to_string());
    }
    if !(1..=128).contains(&m_actual) {
        return Err("AQ4 WMMA ragged-M prototype requires 1 <= M <= 128".to_string());
    }
    if rows == 0 || cols == 0 || !rows.is_multiple_of(16) || !cols.is_multiple_of(32) {
        return Err(
            "AQ4 WMMA ragged-M prototype requires nonzero rows divisible by 16 and cols divisible by 32"
                .to_string(),
        );
    }
    if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
        return Err(
            "AQ4 WMMA ragged-M prototype tensor scale must be finite and greater than zero"
                .to_string(),
        );
    }
    let grid_x = rows
        .checked_div(16)
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype row grid is invalid".to_string())?;
    if grid_x == 0 || grid_x > u32::MAX as usize {
        return Err("AQ4 WMMA ragged-M prototype dimensions exceed HIP grid limit".to_string());
    }
    let elements = rows
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype matrix element count overflows".to_string())?;
    let index_bytes = elements / 2;
    let groups = elements / group_size;
    let scale_value_bytes = scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype scale value byte size overflows".to_string())?;
    let input_elements = m_actual
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype input element count overflows".to_string())?;
    let output_elements = m_actual
        .checked_mul(rows)
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype output element count overflows".to_string())?;
    let input_bytes = input_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype input byte size overflows".to_string())?;
    let output_bytes = output_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype output byte size overflows".to_string())?;
    let row_scale_bytes = row_scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA ragged-M prototype row scale byte size overflows".to_string())?;
    check_copy_range(0, index_bytes, index_buffer.size()?)?;
    check_copy_range(0, groups, scale_buffer.size()?)?;
    check_copy_range(0, 16 * std::mem::size_of::<f32>(), codebook_buffer.size()?)?;
    check_copy_range(0, scale_value_bytes, scale_values_buffer.size()?)?;
    ...
    let row_scale_raw = row_scale_buffer
        ...
        .unwrap_or(std::ptr::null_mut());
    status_to_result(unsafe {
        ullm_runtime_aq4_matvec_batch_wmma_ragged_m_prototype_f32(
            index_buffer.raw.as_ptr(),
            scale_buffer.raw.as_ptr(),
            codebook_buffer.raw.as_ptr(),
            scale_values_buffer.raw.as_ptr(),
...
/// Directly invokes the isolated v3 gfx1201 rocWMMA AQ4 group16 M=128 experiment.
///
/// V3 is intentionally separate from production dispatch and requires two complete Wide-K
/// microtiles per LDS stage, so cols must be divisible by 64. It has no fallback.
#[allow(clippy::too_many_arguments)]
pub fn aq4_matvec_batch_wmma_prototype_v3_f32(
    index_buffer: &RuntimeBuffer,
    scale_buffer: &RuntimeBuffer,
    codebook_buffer: &RuntimeBuffer,
    scale_values_buffer: &RuntimeBuffer,
    ...
    if scale_count == 0 {
        return Err("AQ4 WMMA prototype v3 scale table is empty".to_string());
    }
    if group_size != 16 {
        return Err("AQ4 WMMA prototype v3 requires group size 16".to_string());
    }
    if batch_count != 128 {
        return Err("AQ4 WMMA prototype v3 requires batch count M=128".to_string());
    }
    if rows == 0 || cols == 0 || !rows.is_multiple_of(16) || !cols.is_multiple_of(64) {
        return Err(
            "AQ4 WMMA prototype v3 requires nonzero rows divisible by 16 and cols divisible by 64"
                .to_string(),
        );
    }
    if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
        return Err(
            "AQ4 WMMA prototype v3 tensor scale must be finite and greater than zero".to_string(),
        );
    }
    let grid_x = rows
        .checked_div(16)
        .ok_or_else(|| "AQ4 WMMA prototype v3 row grid is invalid".to_string())?;
    let grid_y = batch_count
        .checked_div(128)
        .ok_or_else(|| "AQ4 WMMA prototype v3 batch grid is invalid".to_string())?;
    if grid_x == 0 || grid_y == 0 || grid_x > u32::MAX as usize || grid_y > u32::MAX as usize {
        return Err("AQ4 WMMA prototype v3 dimensions exceed HIP grid limit".to_string());
    }
    let elements = rows
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 WMMA prototype v3 matrix element count overflows".to_string())?;
    let index_bytes = elements / 2;
    let groups = elements / group_size;
    let scale_value_bytes = scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v3 scale value byte size overflows".to_string())?;
    let input_elements = batch_count
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 WMMA prototype v3 input element count overflows".to_string())?;
    let output_elements = batch_count
        .checked_mul(rows)
        .ok_or_else(|| "AQ4 WMMA prototype v3 output element count overflows".to_string())?;
    let input_bytes = input_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v3 input byte size overflows".to_string())?;
    let output_bytes = output_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v3 output byte size overflows".to_string())?;
    let row_scale_bytes = row_scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 WMMA prototype v3 row scale byte size overflows".to_string())?;
    check_copy_range(0, index_bytes, index_buffer.size()?)?;
    check_copy_range(0, groups, scale_buffer.size()?)?;
    check_copy_range(0, 16 * std::mem::size_of::<f32>(), codebook_buffer.size()?)?;
    check_copy_range(0, scale_value_bytes, scale_values_buffer.size()?)?;
    ...
    let row_scale_raw = row_scale_buffer
        ...
        .unwrap_or(std::ptr::null_mut());
    status_to_result(unsafe {
        ullm_runtime_aq4_matvec_batch_wmma_prototype_v3_f32(
            index_buffer.raw.as_ptr(),
            scale_buffer.raw.as_ptr(),
            codebook_buffer.raw.as_ptr(),
            scale_values_buffer.raw.as_ptr(),
...
pub fn aq4_matvec_batch_wmma_group8_prototype_f32(
    ...
    if scale_count == 0 {
        return Err("AQ4 group8 WMMA prototype scale table is empty".to_string());
    }
    if group_size != 8 {
        return Err("AQ4 group8 WMMA prototype requires group size 8".to_string());
    }
    if batch_count != 128 {
    ...
    if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
        return Err(
            "AQ4 group8 WMMA prototype tensor scale must be finite and greater than zero"
                .to_string(),
        );
    }
    let grid_x = rows
        .checked_div(16)
        .ok_or_else(|| "AQ4 group8 WMMA prototype row grid is invalid".to_string())?;
    let grid_y = batch_count
        .checked_div(128)
        .ok_or_else(|| "AQ4 group8 WMMA prototype batch grid is invalid".to_string())?;
    if grid_x == 0 || grid_y == 0 || grid_x > u32::MAX as usize || grid_y > u32::MAX as usize {
        return Err("AQ4 group8 WMMA prototype dimensions exceed HIP grid limit".to_string());
    }
    let elements = rows
        .checked_mul(cols)
        .ok_or_else(|| "AQ4 group8 WMMA prototype matrix element count overflows".to_string())?;
    let index_bytes = elements / 2;
    let groups = elements / group_size;
    let scale_value_bytes = scale_count
        .checked_mul(std::mem::size_of::<f32>())
    ...
    let output_bytes = output_elements
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 group8 WMMA prototype output byte size overflows".to_string())?;
    let row_scale_bytes = row_scale_count
        .checked_mul(std::mem::size_of::<f32>())
        .ok_or_else(|| "AQ4 group8 WMMA prototype row scale byte size overflows".to_string())?;
    check_copy_range(0, index_bytes, index_buffer.size()?)?;
    check_copy_range(0, groups, scale_buffer.size()?)?;
    check_copy_range(0, 16 * std::mem::size_of::<f32>(), codebook_buffer.size()?)?;
    check_copy_range(0, scale_value_bytes, scale_values_buffer.size()?)?;
    ...
    let row_scale_raw = row_scale_buffer
        ...
        .unwrap_or(std::ptr::null_mut());
    status_to_result(unsafe {
        ullm_runtime_aq4_matvec_batch_wmma_group8_prototype_f32(
            index_buffer.raw.as_ptr(),
            scale_buffer.raw.as_ptr(),
            codebook_buffer.raw.as_ptr(),
            scale_values_buffer.raw.as_ptr(),
...
/// Direct-only gfx1201 AQ4 matvec-add wide-load prototype. It accepts group8/group16 matrices
/// with complete 32-element chunks and never changes production dispatch or CPU fallback.
#[allow(clippy::too_many_arguments)]
pub fn aq4_matvec_add_wide_load_prototype_f32(
    index_buffer: &RuntimeBuffer,
    scale_buffer: &RuntimeBuffer,
    codebook_buffer: &RuntimeBuffer,
    scale_values_buffer: &RuntimeBuffer,
    ...
    check_copy_range(0, output_bytes, output_buffer.size()?)?;
    let stream = stream.map_or(std::ptr::null_mut(), |stream| stream.raw.as_ptr());
    let row_scale_raw = row_scale_buffer.map_or(std::ptr::null(), |buffer| buffer.raw.as_ptr());
    status_to_result(unsafe {
        ullm_runtime_aq4_matvec_add_wide_load_prototype_f32(
            index_buffer.raw.as_ptr(),
            scale_buffer.raw.as_ptr(),
            codebook_buffer.raw.as_ptr(),
            scale_values_buffer.raw.as_ptr(),
...
/// Direct-only gfx1201 AQ4 matvec-add shuffle prototype. It preserves the promoted uint4 weight
/// traversal and residual-add epilogue while requiring the exact RPB=8 wave32 geometry.
#[allow(clippy::too_many_arguments)]
pub fn aq4_matvec_add_shuffle_prototype_f32(
    index_buffer: &RuntimeBuffer,
    scale_buffer: &RuntimeBuffer,
    codebook_buffer: &RuntimeBuffer,
    scale_values_buffer: &RuntimeBuffer,
    ...
    if row_scale_count != 0 && row_scale_count != rows {
        return Err("AQ4 matvec add shuffle prototype row_scale_count must be zero or rows".into());
    }
    if row_scale_count != 0 && row_scale_buffer.is_none() {
        return Err(
            "AQ4 matvec add shuffle prototype row_scale_count requires a row-scale buffer".into(),
        );
    }
    if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
    ...
    let row_scale_raw = row_scale_buffer.map_or(std::ptr::null(), |buffer| buffer.raw.as_ptr());
    status_to_result(unsafe {
        ullm_runtime_aq4_matvec_add_shuffle_prototype_f32(
            index_buffer.raw.as_ptr(),
            scale_buffer.raw.as_ptr(),
            codebook_buffer.raw.as_ptr(),
            scale_values_buffer.raw.as_ptr(),
...
pub fn aq4_matvec_qkv_z_gate_beta_wide_load_prototype_f32(
    ...
    for (label, row_scale, row_scale_count) in [
        ...
        ("a", a_row_scale, a_row_scale_count),
        ("b", b_row_scale, b_row_scale_count),
    ] {
        if row_scale_count != 0 && row_scale.is_none() {
            return Err(format!(
                "AQ4 wide-load fused qkv/z gate/beta {label} row_scale_count requires a row-scale buffer"
            ));
        }
    }
...
fn paged_causal_gqa_chunk_wmma_shape_is_supported(
    ...
    block_size == 256 && q_heads == 16 && kv_heads == 4 && head_dim == 256 && value_dim == 256
}
