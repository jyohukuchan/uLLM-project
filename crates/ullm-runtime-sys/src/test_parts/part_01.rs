    ...
    fn hip_aq4_wmma_prototype_v2_m128_group16_mlp_shapes_match_cpu_when_enabled() {
        assert_eq!(
            ...
            Ok("1"),
            "set ULLM_RUN_AQ4_WMMA_PROTOTYPE_V2_DIFFERENTIAL=1 before running this GPU differential test"
        );
        let device_index = (1..device_count().unwrap())
            .find(|&candidate| {
                device_info(candidate)
        ...
        for &(family, rows, cols, use_row_scales) in &[
            ...
            let actual = {
                ...
                hip_stream.synchronize().unwrap();
                aq4_matvec_batch_wmma_prototype_v2_f32(
                    &hip_index,
                    &hip_scale,
                    &hip_codebook,
                    &hip_scale_values,
    ...
    fn hip_aq4_wmma_prototype_v2_m128_group16_mlp_timing_vs_wmma_prototype_when_enabled() {
        assert_eq!(
            ...
            Ok("1"),
            "set ULLM_RUN_AQ4_WMMA_PROTOTYPE_V2_TIMING=1 before running this GPU timing test"
        );
        let device_index = (1..device_count().unwrap())
            .find(|&candidate| {
                device_info(candidate)
        ...
        for &(family, rows, cols) in &[
            ...
            // Compile/load both isolated WMMA modules and reach steady state before timing.
            for _ in 0..WARMUP_ITERATIONS {
                aq4_matvec_batch_wmma_prototype_f32(
                    &hip_index,
                    &hip_scale,
            ...
            hip_stream.synchronize().unwrap();
            let v1_elapsed = v1_started.elapsed();
            ...
            hip_stream.synchronize().unwrap();
            let v2_elapsed = v2_started.elapsed();

            let v1_ms = v1_elapsed.as_secs_f64() * 1_000.0 / TIMED_ITERATIONS as f64;
            let v2_ms = v2_elapsed.as_secs_f64() * 1_000.0 / TIMED_ITERATIONS as f64;
            let flops_per_launch = 2.0_f64 * rows as f64 * cols as f64 * batch_count as f64;
            let v1_tflops = flops_per_launch / (v1_ms / 1_000.0) / 1.0e12;
            let v2_tflops = flops_per_launch / (v2_ms / 1_000.0) / 1.0e12;
            assert!(v1_ms.is_finite() && v1_ms > 0.0);
            assert!(v2_ms.is_finite() && v2_ms > 0.0);
            eprintln!(
                "AQ4 WMMA prototype v2 timing family={family} rows={rows} cols={cols} M={batch_count}: v1={v1_ms:.3} ms ({v1_tflops:.2} TFLOPS), v2={v2_ms:.3} ms ({v2_tflops:.2} TFLOPS), speedup={:.3}x",
                v1_ms / v2_ms
            );
        }
    }
