    ...
    fn hip_aq4_wmma_prototype_m128_group16_model_shapes_match_cpu_when_enabled() {
        ...
        // This stable direct ABI is the one selected by HipAq4GemmWmmaF32. It compiles the
        // production double-buffered HIPRTC source, so this test covers the actual served kernel.
    ...
    fn hip_aq4_wmma_prototype_v4_m128_group16_model_shapes_match_cpu_when_enabled() {
        ...
        let mut seed = 0x8f1b_bcdd_u32;
        let mut next_unit = || {
            seed = seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223);
            (seed as f32) / (u32::MAX as f32)
        };
        // V3 is direct-only and owns a separate HIPRTC module/cache. Cover the complete group16
        // production shape set, including both row-scale ABI states and the two-row-tile case.
        for &(family, rows, cols, use_row_scales) in &[
            ("attn_q + linear_attn_qkv", 8_192_usize, 4_096, true),
            ("linear_attn_z", 4_096, 4_096, false),
            ("linear_attn_a + linear_attn_b", 32, 4_096, true),
            ...
            let batch_count = 128_usize;
            assert_eq!(rows % 16, 0, "{family} must fill WMMA row tiles");
            assert_eq!(cols % 64, 0, "{family} must fill v3 two-Wide-K stages");
            if rows == 32 {
                assert_eq!(rows / 16, 2, "{family} must launch exactly two row tiles");
            }
            let elements = rows * cols;
            ...
            let scale_values: Vec<f32> =
                (0..scale_count).map(|_| 0.5 + next_unit() * 0.5).collect();
            let input_values: Vec<f32> = (0..batch_count * cols)
                .map(|_| next_unit() * 2.0 - 1.0)
                .collect();
            let row_scales: Vec<f32> = (0..rows).map(|_| 0.75 + next_unit() * 0.5).collect();
            ...
            let expected = {
                ...
                let mut expected_bytes =
                    vec![0_u8; batch_count * rows * std::mem::size_of::<f32>()];
                cpu_output
                    .copy_to_host(0, &mut expected_bytes, Some(&mut cpu_stream))
                    .unwrap();
                cpu_stream.synchronize().unwrap();
            ...
            // Both paths round decoded weights and activations to FP16 before a FP32 WMMA
            // accumulation. Keep the production WMMA differential tolerance unchanged.
            for (index, (actual, expected)) in actual.iter().zip(&expected).enumerate() {
                let tolerance = 5e-2_f32 + 1e-2_f32 * expected.abs();
                assert!(
                    (actual - expected).abs() <= tolerance,
    ...
    fn hip_aq4_wmma_prototype_v4_m128_group16_model_shapes_timing_vs_wmma_prototype_when_enabled() {
        ...
        const WARMUP_ITERATIONS: usize = 3;
        const TIMED_ITERATIONS: usize = 20;
        let mut seed = 0x3c6e_f372_u32;
        let mut next_unit = || {
            seed = seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223);
            (seed as f32) / (u32::MAX as f32)
        };
        // Keep the full production group16 set in the A/B harness. The two MLP geometries are
        // the promotion decision makers, while the others expose small-grid or K-depth regressions.
        for &(family, rows, cols) in &[
            ("attn_q + linear_attn_qkv", 8_192_usize, 4_096),
            ("linear_attn_z", 4_096, 4_096),
            ("linear_attn_a + linear_attn_b", 32, 4_096),
            ...
            // Compile/load both isolated modules and reach steady-state allocations before
            // recording either side. Production uses today's promoted double-buffered v2 ABI.
            for _ in 0..WARMUP_ITERATIONS {
                aq4_matvec_batch_wmma_prototype_f32(
                    &hip_index,
                    &hip_scale,
            ...
            hip_stream.synchronize().unwrap();
            let v3_elapsed = v3_started.elapsed();

            let production_ms =
                production_elapsed.as_secs_f64() * 1_000.0 / TIMED_ITERATIONS as f64;
            let v3_ms = v3_elapsed.as_secs_f64() * 1_000.0 / TIMED_ITERATIONS as f64;
            let flops_per_launch = 2.0_f64 * rows as f64 * cols as f64 * batch_count as f64;
            let production_tflops = flops_per_launch / (production_ms / 1_000.0) / 1.0e12;
            let v3_tflops = flops_per_launch / (v3_ms / 1_000.0) / 1.0e12;
            assert!(production_ms.is_finite() && production_ms > 0.0);
            assert!(v3_ms.is_finite() && v3_ms > 0.0);
            eprintln!(
                "AQ4 WMMA prototype v3 timing family={family} rows={rows} cols={cols} M={batch_count}: production-v2={production_ms:.3} ms ({production_tflops:.2} TFLOPS), v3={v3_ms:.3} ms ({v3_tflops:.2} TFLOPS), speedup={:.3}x",
                production_ms / v3_ms
            );
        }
    }
    ...
    fn hip_aq4_wmma_group8_m128_target_shapes_match_cpu_when_enabled() {
        ...
        for &(family, rows, cols, use_row_scales) in &[
            ...
            // The CPU AQ4 reference remains FP32. This kernel stages both decoded weights and
            // activations through FP16 before FP32 WMMA accumulation, so use the established
            // bounded 0.05 absolute + 1% relative allowance. It is deliberately far below the
            // error expected from a group-scale, nibble-order, transpose, or factor-placement bug.
            for (index, (actual, expected)) in actual.iter().zip(&expected).enumerate() {
                let tolerance = 5e-2_f32 + 1e-2_f32 * expected.abs();
                assert!(
                    (actual - expected).abs() <= tolerance,
    ...
    fn hip_aq4_wmma_group8_m128_target_shapes_timing_vs_register_bm8_group8_when_enabled()
    {
        ...
        for &(family, rows, cols) in &[
            ...
            // Compile/load both independent modules and reach steady-state allocations before
            // wall timing. The scalar side is the current production group8 register kernel.
            for _ in 0..WARMUP_ITERATIONS {
                aq4_matvec_batch_register_bm8_group8_f32(
                    &hip_index,
                    &hip_scale,
