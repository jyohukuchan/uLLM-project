...
fn package_token_ids_mixed_request_state_smoke_impl_with_sq_overlay(
    ...
    let sq_projection_kernel_families = sq_fp8_projection_kernel_families(
        sq_fp8_projection_telemetry,
        sq_fp8_projection_dispatches,
    );
    ...
    let sq_format_id = sq_overlay_info
        .as_ref()
        .map(|info| info.format_id.as_str())
        .unwrap_or("none");
