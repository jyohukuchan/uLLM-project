...
#[cfg(test)]
std::thread_local! {
    // Fault injection is deliberately thread-local. Library tests run in parallel, while the
    // probe and its checkpoints run synchronously on the requesting thread; a process-global
    // stage would let an unrelated test consume the requested fault.
    static FORCE_M1_PROBE_FAILURE_STAGE: std::cell::Cell<u8> =
        const { std::cell::Cell::new(ProbeFaultStage::None as u8) };
...
fn force_probe_failure(stage: ProbeFaultStage) {
    FORCE_M1_PROBE_FAILURE_STAGE.with(|forced| forced.set(stage as u8));
}
...
fn probe_fault_checkpoint(stage: u8, label: &str) -> Result<(), String> {
    ...
    #[cfg(test)]
    if FORCE_M1_PROBE_FAILURE_STAGE.with(|forced| {
        if forced.get() == stage {
            forced.set(ProbeFaultStage::None as u8);
            true
        } else {
            false
        }
...
const fn aq4_wmma_probe_binding(batch_count: usize) -> Aq4ProbeBinding {
    Aq4ProbeBinding {
        scale_count: 2,
        group_size: 16,
        tensor_scale_bits: 1.0_f32.to_bits(),
        row_scale_count: 0,
        // A real linear_attn_a/b projection. This specifically exercises the generalized
        // small-row WMMA launch: rows / 16 == 2 and M / 128 == 1.
        rows: 32,
        cols: 4096,
        batch_count,
    }
...
const fn aq4_wmma_group8_probe_binding(batch_count: usize) -> Aq4ProbeBinding {
    Aq4ProbeBinding {
        scale_count: 2,
        group_size: 8,
        tensor_scale_bits: 1.0_f32.to_bits(),
        row_scale_count: 0,
        // A real group8 attn_k/v projection. This proves the separately compiled rocWMMA
        // module on one of the exact served M=128 shapes before dispatch can select it.
        rows: 1_024,
        cols: 4_096,
        batch_count,
    }
...
impl DeviceCapabilities {
    ...
    pub fn probe_m1_runtime_context(
        ...
        if policy.contains(RuntimeFeature::HipAq4GemmWmma) {
            // Probe an actual small-row group16 projection, not just an MLP shape. This covers
            // the generalized exact grid (rows / 16 = 2, M / 128 = 1) and rocWMMA HIPRTC path.
            let binding = aq4_wmma_probe_binding(128);
            let packed_indices = aq4_probe_packed_indices(binding)?;
            let scale_indices = aq4_probe_scale_indices(binding)?;
            let codebook_bytes = aq4_probe_codebook_bytes();
            let scale_value_bytes = aq4_probe_scale_value_bytes();
            ...
            let input = zeros(context, stream, binding.input_elements()?)?;
            let mut output = zeros(context, stream, binding.output_elements()?)?;
            ullm_runtime_sys::aq4_matvec_batch_wmma_prototype_f32(
                &index,
                &scale,
                &codebook,
                &scale_values,
            ...
            probe_fault_checkpoint(18, "aq4-gemm-wmma")?;
            proven = proven.with(RuntimeFeature::HipAq4GemmWmma);
        }
        if policy.contains(RuntimeFeature::HipAq4GemmWmmaGroup8) {
            // Probe an actual group8 target projection before M=128 plans can select this
            // separately compiled rocWMMA module. The descriptor gate itself remains the exact
            // generic 16-row/32-column alignment contract shared with the kernel.
            let binding = aq4_wmma_group8_probe_binding(128);
            let packed_indices = aq4_probe_packed_indices(binding)?;
            let scale_indices = aq4_probe_scale_indices(binding)?;
            let codebook_bytes = aq4_probe_codebook_bytes();
            let scale_value_bytes = aq4_probe_scale_value_bytes();
            ...
            let input = zeros(context, stream, binding.input_elements()?)?;
            let mut output = zeros(context, stream, binding.output_elements()?)?;
            ullm_runtime_sys::aq4_matvec_batch_wmma_group8_prototype_f32(
                &index,
                &scale,
                &codebook,
                &scale_values,
            ...
            probe_fault_checkpoint(20, "aq4-gemm-wmma-group8")?;
            proven = proven.with(RuntimeFeature::HipAq4GemmWmmaGroup8);
        }
        if policy.contains(RuntimeFeature::HipAq4GemmWmmaRaggedM) {
            // Probe the largest validated ragged width on a real group16 projection. The
            // independently guarded feature admits only M=65..=127 registry descriptors.
            let binding = aq4_wmma_probe_binding(127);
            let packed_indices = aq4_probe_packed_indices(binding)?;
            let scale_indices = aq4_probe_scale_indices(binding)?;
            let codebook_bytes = aq4_probe_codebook_bytes();
            let scale_value_bytes = aq4_probe_scale_value_bytes();
            ...
            let input = zeros(context, stream, binding.input_elements()?)?;
            let mut output = zeros(context, stream, binding.output_elements()?)?;
            ullm_runtime_sys::aq4_matvec_batch_wmma_ragged_m_prototype_f32(
                &index,
                &scale,
                &codebook,
                &scale_values,
...
impl StartedOperationPlan<'_> {
    ...
    pub fn execute_aq4_gemm_wmma_f32(
        self,
        index: &ullm_runtime_sys::RuntimeBuffer,
        scale: &ullm_runtime_sys::RuntimeBuffer,
        codebook: &ullm_runtime_sys::RuntimeBuffer,
        ...
        let OperationGeometry::Aq4MatvecBatch {
            ...
            return Err("resolved AQ4 WMMA GEMM operation has incompatible geometry".into());
        };
        if plan.kind != OperationKind::Aq4MatvecBatch
            || plan.executable != ExecutableOperation::HipAq4GemmWmmaF32
            || plan.device.backend != OperationBackend::Hip
            || plan.device.architecture.as_deref() != Some("gfx1201")
            || plan.batch_width != 128
            || plan.chunk_width != 1
        ...
        {
            return Err(format!(
                "resolved backend operation {} is not a gfx1201/group16/M=128 AQ4 WMMA GEMM operation",
                plan.implementation_id
            ));
        }
        if scale_count == 0 {
            return Err("resolved AQ4 WMMA GEMM operation has zero scale count".into());
        }
        let tensor_scale = f32::from_bits(tensor_scale_bits);
        if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
            return Err("resolved AQ4 WMMA GEMM tensor scale is invalid".into());
        }
        if row_scale_count == 0 && row_scale.is_some() {
            return Err("resolved AQ4 WMMA GEMM has an unexpected row scale buffer".into());
        }
        if row_scale_count != 0 && row_scale.is_none() {
            return Err("resolved AQ4 WMMA GEMM is missing its row scale buffer".into());
        }
        let batch_count = usize::try_from(plan.batch_width)
            .map_err(|_| "resolved AQ4 WMMA GEMM width exceeds usize".to_string())?;
        ullm_runtime_sys::aq4_matvec_batch_wmma_prototype_f32(
            index,
            scale,
            codebook,
    ...
    /// Executes the production gfx1201/group16 ragged-M rocWMMA AQ4 GEMM ABI selected at load
    /// time.
    ///
    /// This method intentionally has no fallback path: a plan can select this ABI only after its
    /// independent ragged-M probe succeeds, and ABI failures must remain visible to the caller.
    pub fn execute_aq4_gemm_wmma_ragged_m_f32(
        self,
        index: &ullm_runtime_sys::RuntimeBuffer,
        scale: &ullm_runtime_sys::RuntimeBuffer,
        codebook: &ullm_runtime_sys::RuntimeBuffer,
        ...
        let OperationGeometry::Aq4MatvecBatch {
            ...
            return Err(
                "resolved AQ4 ragged-M WMMA GEMM operation has incompatible geometry".into(),
            );
        };
        if plan.kind != OperationKind::Aq4MatvecBatch
            || plan.executable != ExecutableOperation::HipAq4GemmWmmaRaggedMF32
            || plan.device.backend != OperationBackend::Hip
            || plan.device.architecture.as_deref() != Some("gfx1201")
        ...
        {
            return Err(format!(
                "resolved backend operation {} is not a gfx1201/group16/M=65..=127 AQ4 ragged-M WMMA GEMM operation",
                plan.implementation_id
            ));
        }
        if scale_count == 0 {
            return Err("resolved AQ4 ragged-M WMMA GEMM operation has zero scale count".into());
        }
        let tensor_scale = f32::from_bits(tensor_scale_bits);
        if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
            return Err("resolved AQ4 ragged-M WMMA GEMM tensor scale is invalid".into());
        }
        if row_scale_count == 0 && row_scale.is_some() {
            return Err(
                "resolved AQ4 ragged-M WMMA GEMM has an unexpected row scale buffer".into(),
            );
        }
        if row_scale_count != 0 && row_scale.is_none() {
            return Err("resolved AQ4 ragged-M WMMA GEMM is missing its row scale buffer".into());
        }
        let batch_count = usize::try_from(plan.batch_width)
            .map_err(|_| "resolved AQ4 ragged-M WMMA GEMM width exceeds usize".to_string())?;
        ullm_runtime_sys::aq4_matvec_batch_wmma_ragged_m_prototype_f32(
            index,
            scale,
            codebook,
    ...
    /// must fail closed if the independently probed rocWMMA ABI cannot execute.
    pub fn execute_aq4_gemm_wmma_group8_f32(
        self,
        index: &ullm_runtime_sys::RuntimeBuffer,
        scale: &ullm_runtime_sys::RuntimeBuffer,
        codebook: &ullm_runtime_sys::RuntimeBuffer,
        ...
        let OperationGeometry::Aq4MatvecBatch {
            ...
            return Err("resolved AQ4 group8 WMMA GEMM operation has incompatible geometry".into());
        };
        if plan.kind != OperationKind::Aq4MatvecBatch
            || plan.executable != ExecutableOperation::HipAq4GemmWmmaGroup8F32
            || plan.device.backend != OperationBackend::Hip
            || plan.device.architecture.as_deref() != Some("gfx1201")
            || plan.batch_width != 128
            || plan.chunk_width != 1
        ...
        {
            return Err(format!(
                "resolved backend operation {} is not a gfx1201/group8/M=128 AQ4 WMMA GEMM operation",
                plan.implementation_id
            ));
        }
        if scale_count == 0 {
            return Err("resolved AQ4 group8 WMMA GEMM operation has zero scale count".into());
        }
        let tensor_scale = f32::from_bits(tensor_scale_bits);
        if !tensor_scale.is_finite() || tensor_scale <= 0.0 {
            return Err("resolved AQ4 group8 WMMA GEMM tensor scale is invalid".into());
        }
        if row_scale_count == 0 && row_scale.is_some() {
            return Err("resolved AQ4 group8 WMMA GEMM has an unexpected row scale buffer".into());
        }
        if row_scale_count != 0 && row_scale.is_none() {
            return Err("resolved AQ4 group8 WMMA GEMM is missing its row scale buffer".into());
        }
        let batch_count = usize::try_from(plan.batch_width)
            .map_err(|_| "resolved AQ4 group8 WMMA GEMM width exceeds usize".to_string())?;
        ullm_runtime_sys::aq4_matvec_batch_wmma_group8_prototype_f32(
            index,
            scale,
            codebook,
...
fn validate_descriptor(descriptor: &ImplementationDescriptor) -> Result<(), String> {
    ...
    let semantic_binding_valid = match (descriptor.kind, descriptor.geometry, descriptor.executable)
    {
        ...
        (
            OperationKind::Aq4MatvecBatch,
            OperationGeometry::Aq4MatvecBatch { .. },
            ExecutableOperation::HipAq4GemmWmmaGroup8F32,
        ) => true,
        (
            OperationKind::Aq4MatvecBatch,
            OperationGeometry::Aq4MatvecBatch { .. },
...
fn aq4_matvec_batch_descriptors(
    ...
    let eligible_register_bm8_group16 = device.backend == OperationBackend::Hip
        ...
        && rows.is_multiple_of(32)
        && cols.is_multiple_of(128);
    let eligible_wmma = device.backend == OperationBackend::Hip
        && device.architecture.as_deref() == Some("gfx1201")
        && group_size == 16
        && rows != 0
        && cols != 0
    ...
    let eligible_register_bm8_group8 = device.backend == OperationBackend::Hip
        ...
        && rows.is_multiple_of(32)
        && cols.is_multiple_of(128);
    let eligible_wmma_group8 = device.backend == OperationBackend::Hip
        && device.architecture.as_deref() == Some("gfx1201")
        && group_size == 8
        && rows != 0
        && cols != 0
    ...
    let eligible_wmma_any = eligible_wmma || eligible_wmma_group8;
    let descriptor_count = if eligible_wmma && eligible_register_bm8_group16 {
        4
    } else if eligible_wmma_any && eligible_register_bm8 {
        3
    } else if eligible_wmma_any || eligible_register_bm8 {
        2
    } else {
    ...
    let legacy_architecture = (device.backend == OperationBackend::Hip)
        .then_some("gfx1201")
        .filter(|_| eligible_register_bm8 || eligible_wmma_any);
    if eligible_wmma {
        // The group16 register gate (rows % 32 == 0 && cols % 128 == 0) is a strict subset of
        // the WMMA gate below. Keep M=128 exclusive to exact WMMA, and give the independently
        // guarded ragged M=65..=127 WMMA descriptor a higher priority than register BM8.
        let legacy_maximum_batch_width = if eligible_register_bm8_group16 {
            7
        } else {
            127
        };
        let legacy_id = if eligible_register_bm8_group16 {
            "hip.aq4-matvec-batch-f32.m2-m7"
        } else {
            "hip.aq4-matvec-batch-f32.m2-m127"
        };
        push_descriptor(
            &mut descriptors,
            2,
            legacy_maximum_batch_width,
            legacy_id,
            legacy_features,
            ExecutableOperation::HipAq4MatvecBatchF32,
            legacy_architecture,
        ...
        if eligible_register_bm8_group16 {
            push_descriptor(
                ...
                8,
                127,
                "hip.aq4-gemm-register-bm8-f32.gfx1201.group16.m8-m127",
            ...
            // Keep the full M=8..=127 register descriptor as a lower-priority fallback. The
            // independently guarded ragged WMMA ABI wins only for the timing-validated range.
            push_descriptor(
                &mut descriptors,
                65,
                127,
                "hip.aq4-gemm-wmma-ragged-m-f32.gfx1201.group16.m65-m127",
                RuntimeFeatureSet::from_feature(RuntimeFeature::HipAq4GemmWmmaRaggedM),
                ExecutableOperation::HipAq4GemmWmmaRaggedMF32,
                Some("gfx1201"),
                WorkspaceFormula::ZERO,
                101,
        ...
        push_descriptor(
            ...
            RuntimeFeatureSet::from_feature(RuntimeFeature::HipAq4GemmWmma),
            ExecutableOperation::HipAq4GemmWmmaF32,
            Some("gfx1201"),
            WorkspaceFormula::ZERO,
            100,
        );
    } else if eligible_wmma_group8 {
        // The group8 register gate (rows % 32 == 0 && cols % 128 == 0) is a strict subset of
        // this WMMA gate: its divisors imply the 16-row/32-column alignment, while 16x32 is
        // WMMA-only. Keep M=128 exclusive to WMMA so equal-priority descriptors cannot resolve
        // to the old group8 register path.
        let legacy_maximum_batch_width = if eligible_register_bm8_group8 { 7 } else { 127 };
        let legacy_id = if eligible_register_bm8_group8 {
            "hip.aq4-matvec-batch-f32.m2-m7"
        } else {
            "hip.aq4-matvec-batch-f32.m2-m127"
        };
        ...
        if eligible_register_bm8_group8 {
            push_descriptor(
                &mut descriptors,
                8,
                127,
                "hip.aq4-gemm-register-bm8-group8-f32.gfx1201.group8.m8-m127",
        ...
        push_descriptor(
            ...
            128,
            128,
            "hip.aq4-gemm-wmma-group8-f32.gfx1201.group8.m128",
            RuntimeFeatureSet::from_feature(RuntimeFeature::HipAq4GemmWmmaGroup8),
            ExecutableOperation::HipAq4GemmWmmaGroup8F32,
            Some("gfx1201"),
            WorkspaceFormula::ZERO,
            100,
        );
    } else if eligible_register_bm8 {
        push_descriptor(
            &mut descriptors,
            2,
            7,
...
mod tests {
    ...
    fn test_hip_capabilities() -> DeviceCapabilities {
        DeviceCapabilities {
            ...
            runtime_features: RuntimeFeatureSet::EMPTY
                ...
                .with(RuntimeFeature::HipAq4GemmWmmaGroup8)
                .with(RuntimeFeature::HipAq4GemmWmmaRaggedM),
    ...
    fn eligible_aq4_geometry() -> OperationGeometry {
        wmma_eligible_aq4_geometry(32, 128)
    }

    fn wmma_eligible_aq4_geometry(rows: usize, cols: usize) -> OperationGeometry {
        OperationGeometry::Aq4MatvecBatch {
            rows,
            cols,
            group_size: 16,
    ...
    fn eligible_aq4_group8_geometry(rows: usize) -> OperationGeometry {
        wmma_eligible_aq4_group8_geometry(rows, 4_096)
    }

    fn wmma_eligible_aq4_group8_geometry(rows: usize, cols: usize) -> OperationGeometry {
        OperationGeometry::Aq4MatvecBatch {
            rows,
            cols,
            group_size: 8,
    ...
    #[test]
    fn wmma_eligible_aq4_batch_registry_prefers_ragged_wmma_m65_through_m127() {
        let device = test_hip_capabilities();
        // These are the five distinct group16 dimensions used by the model. The shared
        // dimensions cover the paired projection families as noted in each label.
        for (family, rows, cols) in [
            ("attn_q + linear_attn_qkv", 8_192, 4_096),
            ("linear_attn_z", 4_096, 4_096),
            ("linear_attn_a + linear_attn_b", 32, 4_096),
            ("mlp_gate + mlp_up", 12_288, 4_096),
            ("mlp_down", 4_096, 12_288),
        ] {
            let geometry = wmma_eligible_aq4_geometry(rows, cols);
            ...
            let legacy = registry
                ...
                .unwrap();
            assert_eq!(legacy.minimum_batch_width, 2, "{family}");
            assert_eq!(legacy.maximum_batch_width, 7, "{family}");
            let bm8 = registry
                .implementations()
                .iter()
                .find(|descriptor| {
                ...
                .unwrap();
            assert_eq!(bm8.minimum_batch_width, 8, "{family}");
            assert_eq!(bm8.maximum_batch_width, 127, "{family}");
            assert_eq!(
                bm8.id, "hip.aq4-gemm-register-bm8-f32.gfx1201.group16.m8-m127",
                "{family}"
            );
            assert_eq!(
            ...
            assert_eq!(bm8.workspace.estimate(127, 1).unwrap().temporary_bytes, 0);
            assert_eq!(bm8.workspace.maximum_total_bytes, 0);
            let ragged_wmma = registry
                .implementations()
                .iter()
                .find(|descriptor| {
                    descriptor.executable == ExecutableOperation::HipAq4GemmWmmaRaggedMF32
                })
                .unwrap();
            assert_eq!(ragged_wmma.minimum_batch_width, 65, "{family}");
            assert_eq!(ragged_wmma.maximum_batch_width, 127, "{family}");
            assert_eq!(
                ragged_wmma.id, "hip.aq4-gemm-wmma-ragged-m-f32.gfx1201.group16.m65-m127",
                "{family}"
            );
            assert_eq!(
                ragged_wmma.required_features,
                RuntimeFeatureSet::from_feature(RuntimeFeature::HipAq4GemmWmmaRaggedM),
                "{family}"
            );
            assert_eq!(ragged_wmma.priority, bm8.priority + 1, "{family}");
            assert_eq!(
                ragged_wmma
                    .workspace
                    .estimate(127, 1)
            ...
            assert_eq!(ragged_wmma.workspace.maximum_total_bytes, 0, "{family}");
            let wmma = registry
                .implementations()
                .iter()
                .find(|descriptor| descriptor.executable == ExecutableOperation::HipAq4GemmWmmaF32)
                .unwrap();
            assert_eq!(wmma.minimum_batch_width, 128, "{family}");
            assert_eq!(wmma.maximum_batch_width, 128, "{family}");
            assert_eq!(wmma.id, "hip.aq4-gemm-wmma-f32.gfx1201.group16.m128");
            assert_eq!(
                wmma.required_features,
                RuntimeFeatureSet::from_feature(RuntimeFeature::HipAq4GemmWmma),
                "{family}"
            );
            assert_eq!(wmma.workspace.estimate(128, 1).unwrap().temporary_bytes, 0);
            assert_eq!(wmma.workspace.maximum_total_bytes, 0);
            for (width, executable) in [
                (7, ExecutableOperation::HipAq4MatvecBatchF32),
                (8, ExecutableOperation::HipAq4GemmRegisterBm8F32),
                (64, ExecutableOperation::HipAq4GemmRegisterBm8F32),
                (65, ExecutableOperation::HipAq4GemmWmmaRaggedMF32),
                (113, ExecutableOperation::HipAq4GemmWmmaRaggedMF32),
                (127, ExecutableOperation::HipAq4GemmWmmaRaggedMF32),
                (128, ExecutableOperation::HipAq4GemmWmmaF32),
            ] {
                let request = aq4_matvec_batch_operation_request(
                    ExecutionPhase::Decode,
                    geometry,
            ...
            let mut missing_wmma = device.clone();
            missing_wmma.runtime_features = RuntimeFeatureSet::EMPTY
                .with(RuntimeFeature::HipAq4MatvecBatch)
                .with(RuntimeFeature::HipAq4RegisterBm8);
            let mut missing_ragged_wmma = missing_wmma.clone();
            missing_ragged_wmma.runtime_features = missing_ragged_wmma
                .runtime_features
                .with(RuntimeFeature::HipAq4GemmWmma);
            for width in [65, 113, 127] {
                let request = aq4_matvec_batch_operation_request(
                    ExecutionPhase::Decode,
                    geometry,
                    width,
                    missing_ragged_wmma.clone(),
                    u64::MAX,
                )
                .unwrap();
                assert_eq!(
                    registry.resolve(&request).unwrap().trace().executable,
                    ExecutableOperation::HipAq4GemmRegisterBm8F32,
                    "{family} M={width} falls back without ragged WMMA"
                );
            }
            let request = aq4_matvec_batch_operation_request(
                ExecutionPhase::Decode,
            ...
            .unwrap();
            assert!(registry.resolve(&request).is_err(), "{family}");
        }
    }

    #[test]
    fn wmma_only_aq4_batch_registry_keeps_legacy_through_m127() {
        // A nonzero group16 geometry is register-eligible only when rows % 32 == 0 and
        // cols % 128 == 0. Those conditions imply the WMMA divisors, so this 16x32 example
        // exercises the meaningful asymmetric case: WMMA-only, never register-only.
        let device = test_hip_capabilities();
        let geometry = wmma_eligible_aq4_geometry(16, 32);
        let registry = aq4_matvec_batch_production_registry(geometry, &device).unwrap();
        assert_eq!(registry.implementations().len(), 2);
        let legacy = registry
            .implementations()
        ...
        assert_eq!(legacy.maximum_batch_width, 127);
        assert!(registry.implementations().iter().all(|descriptor| {
            descriptor.executable != ExecutableOperation::HipAq4GemmRegisterBm8F32
        }));
        for (width, executable) in [
            (8, ExecutableOperation::HipAq4MatvecBatchF32),
            (64, ExecutableOperation::HipAq4MatvecBatchF32),
            (127, ExecutableOperation::HipAq4MatvecBatchF32),
            (128, ExecutableOperation::HipAq4GemmWmmaF32),
        ] {
            let request = aq4_matvec_batch_operation_request(
                ExecutionPhase::Decode,
    ...
    #[test]
    fn wmma_eligible_group8_aq4_batch_registry_splits_register_and_wmma_at_m128() {
        let device = test_hip_capabilities();
        for (family, rows) in [
            ("attn_o + linear_attn_out", 4_096_usize),
            ("attn_k + attn_v", 1_024),
        ] {
            ...
            let register = registry
                ...
                .unwrap();
            assert_eq!(
                register.id, "hip.aq4-gemm-register-bm8-group8-f32.gfx1201.group8.m8-m127",
                "{family}"
            );
            assert_eq!(register.minimum_batch_width, 8, "{family}");
            assert_eq!(register.maximum_batch_width, 127, "{family}");
            assert_eq!(
            ...
            assert_eq!(
                register.workspace.estimate(127, 1).unwrap().temporary_bytes,
                0,
                "{family}"
            );
            assert_eq!(register.workspace.maximum_total_bytes, 0, "{family}");
            let wmma = registry
                .implementations()
                .iter()
                .find(|descriptor| {
                    descriptor.executable == ExecutableOperation::HipAq4GemmWmmaGroup8F32
                })
                .unwrap();
            assert_eq!(wmma.minimum_batch_width, 128, "{family}");
            assert_eq!(wmma.maximum_batch_width, 128, "{family}");
            assert_eq!(
                wmma.id, "hip.aq4-gemm-wmma-group8-f32.gfx1201.group8.m128",
                "{family}"
            );
            assert_eq!(
                wmma.required_features,
            ...
            assert_eq!(
                wmma.workspace.estimate(128, 1).unwrap().temporary_bytes,
                0,
                "{family}"
            );
            assert_eq!(wmma.workspace.maximum_total_bytes, 0, "{family}");
            for (width, executable) in [
                (2, ExecutableOperation::HipAq4MatvecBatchF32),
                (7, ExecutableOperation::HipAq4MatvecBatchF32),
                (8, ExecutableOperation::HipAq4GemmRegisterBm8Group8F32),
                (64, ExecutableOperation::HipAq4GemmRegisterBm8Group8F32),
                (127, ExecutableOperation::HipAq4GemmRegisterBm8Group8F32),
                (128, ExecutableOperation::HipAq4GemmWmmaGroup8F32),
            ] {
                let request = aq4_matvec_batch_operation_request(
                    ExecutionPhase::Decode,
                    geometry,
    ...
    #[test]
    fn wmma_only_group8_aq4_batch_registry_keeps_legacy_through_m127() {
        // The group8 register divisors imply the group8 WMMA divisors, but not conversely. This
        // 16x32 geometry exercises the strict WMMA-only side of that relationship.
        let device = test_hip_capabilities();
        let geometry = wmma_eligible_aq4_group8_geometry(16, 32);
        let registry = aq4_matvec_batch_production_registry(geometry, &device).unwrap();
        assert_eq!(registry.implementations().len(), 2);
        let legacy = registry
            .implementations()
    ...
    #[test]
    fn cpu_started_wmma_plan_rejects_before_abi_and_preserves_output() {
        let geometry = eligible_aq4_geometry();
        let hip_registry =
            aq4_matvec_batch_production_registry(geometry, &test_hip_capabilities()).unwrap();
        let descriptor = hip_registry
        ...
        .unwrap();
        let workspace = descriptor.workspace.estimate(128, 1).unwrap();
        let plan = resolved_plan(descriptor, &request, workspace, ResolutionKind::Primary);
        let index = context.alloc_buffer(32 * 128 / 2).unwrap();
        let scale = context.alloc_buffer(32 * 128 / 16).unwrap();
        let codebook = context.alloc_buffer(16 * 4).unwrap();
        let scale_values = context.alloc_buffer(2 * 4).unwrap();
        let input = context.alloc_buffer(128 * 128 * 4).unwrap();
        let mut output = context.alloc_buffer(128 * 32 * 4).unwrap();
        let sentinel = vec![0xa5_u8; 128 * 32 * 4];
        output
            .copy_from_host(0, &sentinel, Some(&mut stream))
            .unwrap();
        stream.synchronize().unwrap();
        let error = plan
            ...
            .unwrap_err();
        assert!(error.contains("gfx1201/group16/M=128"));
        let mut output_bytes = vec![0_u8; sentinel.len()];
        output
            .copy_to_host(0, &mut output_bytes, Some(&mut stream))
            .unwrap();
    ...
    #[test]
    fn cpu_started_group8_wmma_plan_rejects_before_abi_and_preserves_output() {
        let geometry = wmma_eligible_aq4_group8_geometry(16, 32);
        let hip_registry =
            aq4_matvec_batch_production_registry(geometry, &test_hip_capabilities()).unwrap();
        let descriptor = hip_registry
        ...
        let workspace = descriptor.workspace.estimate(128, 1).unwrap();
        let plan = resolved_plan(descriptor, &request, workspace, ResolutionKind::Primary);
        let index = context.alloc_buffer(16 * 32 / 2).unwrap();
        let scale = context.alloc_buffer(16 * 32 / 8).unwrap();
        let codebook = context.alloc_buffer(16 * 4).unwrap();
        let scale_values = context.alloc_buffer(2 * 4).unwrap();
        let input = context.alloc_buffer(128 * 32 * 4).unwrap();
        let mut output = context.alloc_buffer(128 * 16 * 4).unwrap();
        let sentinel = vec![0xa5_u8; 128 * 16 * 4];
        output
            .copy_from_host(0, &sentinel, Some(&mut stream))
            .unwrap();
        stream.synchronize().unwrap();
        let error = plan
            ...
            .unwrap_err();
        assert!(error.contains("gfx1201/group8/M=128"));
        let mut output_bytes = vec![0_u8; sentinel.len()];
        output
            .copy_to_host(0, &mut output_bytes, Some(&mut stream))
            .unwrap();
    ...
    fn every_probe_fault_stage_is_named_and_consumed_once() {
        for (stage, checkpoint, label) in [
            ...
            (ProbeFaultStage::Aq4GemmWmma, 18, "aq4-gemm-wmma"),
            (
                ProbeFaultStage::Aq4GemmWmmaGroup8,
                20,
                "aq4-gemm-wmma-group8",
            ),
            (
                ProbeFaultStage::Aq4GemmWmmaRaggedM,
                21,
                "aq4-gemm-wmma-ragged-m",
            ),
    ...
    #[test]
    fn aq4_wmma_feature_environment_and_fault_checkpoint_are_exact() {
        assert_eq!(
            runtime_feature_environment(RuntimeFeature::HipAq4GemmWmma),
            "ULLM_REQUIRE_HIP_AQ4_WMMA_GEMM_KERNEL"
        );
        let before = M1_PROBE_CHECKPOINT_COUNTS[18].load(std::sync::atomic::Ordering::Acquire);
        force_probe_failure(ProbeFaultStage::Aq4GemmWmma);
        assert!(probe_fault_checkpoint(18, "aq4-gemm-wmma").is_err());
        assert_eq!(
            M1_PROBE_CHECKPOINT_COUNTS[18].load(std::sync::atomic::Ordering::Acquire),
            before + 1
        );
    ...
    #[test]
    fn aq4_group8_wmma_feature_environment_and_fault_checkpoint_are_exact() {
        assert_eq!(
            runtime_feature_environment(RuntimeFeature::HipAq4GemmWmmaGroup8),
            "ULLM_REQUIRE_HIP_AQ4_WMMA_GEMM_GROUP8_KERNEL"
        );
        let before = M1_PROBE_CHECKPOINT_COUNTS[20].load(std::sync::atomic::Ordering::Acquire);
        force_probe_failure(ProbeFaultStage::Aq4GemmWmmaGroup8);
        assert!(probe_fault_checkpoint(20, "aq4-gemm-wmma-group8").is_err());
        assert_eq!(
            M1_PROBE_CHECKPOINT_COUNTS[20].load(std::sync::atomic::Ordering::Acquire),
            before + 1
        );
        probe_fault_checkpoint(20, "aq4-gemm-wmma-group8").unwrap();
    }

    #[test]
    fn aq4_ragged_m_wmma_feature_environment_and_fault_checkpoint_are_exact() {
        assert_eq!(
            runtime_feature_environment(RuntimeFeature::HipAq4GemmWmmaRaggedM),
            "ULLM_REQUIRE_HIP_AQ4_WMMA_GEMM_RAGGED_M_KERNEL"
        );
        let before = M1_PROBE_CHECKPOINT_COUNTS[21].load(std::sync::atomic::Ordering::Acquire);
        force_probe_failure(ProbeFaultStage::Aq4GemmWmmaRaggedM);
        assert!(probe_fault_checkpoint(21, "aq4-gemm-wmma-ragged-m").is_err());
        assert_eq!(
            M1_PROBE_CHECKPOINT_COUNTS[21].load(std::sync::atomic::Ordering::Acquire),
            before + 1
        );
        probe_fault_checkpoint(21, "aq4-gemm-wmma-ragged-m").unwrap();
    }
    ...
    fn isolated_hip_aq4_wmma_probe_enables_production_m128_dispatch_when_guarded() {
        ...
        assert!(
            device
                .runtime_features
                .contains(RuntimeFeature::HipAq4GemmWmma)
        );
    ...
    #[test]
    #[ignore = "requires an isolated gfx1201 HIP device and ULLM_REQUIRE_HIP_AQ4_WMMA_GEMM_RAGGED_M_KERNEL=1"]
    fn isolated_hip_aq4_ragged_m_wmma_probe_enables_production_m127_dispatch_when_guarded() {
        assert_eq!(
        ...
        assert!(
            device
                .runtime_features
                .contains(RuntimeFeature::HipAq4GemmWmmaRaggedM)
        );
    ...
    #[test]
    #[ignore = "requires an isolated gfx1201 HIP device and ULLM_REQUIRE_HIP_AQ4_WMMA_GEMM_GROUP8_KERNEL=1"]
    fn isolated_hip_aq4_group8_wmma_probe_enables_production_m128_dispatch_when_guarded() {
        assert_eq!(
            std::env::var(runtime_feature_environment(
                RuntimeFeature::HipAq4GemmWmmaGroup8
            ))
        ...
        assert!(
            device
                .runtime_features
                .contains(RuntimeFeature::HipAq4GemmWmmaGroup8)
        );

        let geometry = eligible_aq4_group8_geometry(1_024);
        let registry = aq4_matvec_batch_production_registry(geometry, &device).unwrap();
        let request = aq4_matvec_batch_operation_request(
            ExecutionPhase::ColdPrefill,
            geometry,
    ...
    #[test]
    fn aq4_wmma_m128_probe_binding_matches_small_row_geometry_and_packed_layout() {
        let binding = aq4_wmma_probe_binding(128);
        assert_eq!(binding.scale_count, 2);
        assert_eq!(binding.group_size, 16);
        assert_eq!(binding.tensor_scale_bits, 1.0_f32.to_bits());
        assert_eq!(binding.row_scale_count, 0);
        assert_eq!(binding.rows, 32);
        assert_eq!(binding.cols, 4_096);
        assert_eq!(binding.batch_count, 128);
        assert_eq!(binding.rows / 16, 2, "WMMA grid.x");
        assert_eq!(binding.batch_count / 128, 1, "WMMA grid.y");
        assert_eq!(binding.matrix_elements().unwrap(), 131_072);
        assert_eq!(binding.packed_index_bytes().unwrap(), 65_536);
        assert_eq!(binding.scale_index_bytes().unwrap(), 8_192);
        assert_eq!(binding.input_elements().unwrap(), 524_288);
        assert_eq!(binding.output_elements().unwrap(), 4_096);
        assert_eq!(binding.packed_index_bytes().unwrap() % 16, 0);
        assert_eq!(binding.cols * std::mem::size_of::<f32>() % 16, 0);
        let scale_indices = aq4_probe_scale_indices(binding).unwrap();
        assert_eq!(scale_indices.len(), 8_192);
        assert!(
            scale_indices
                .iter()
                .all(|&index| usize::from(index) < binding.scale_count)
        ...
        force_probe_failure(ProbeFaultStage::Aq4GemmWmma);
        let error = probe_fault_checkpoint(18, "aq4-gemm-wmma").unwrap_err();
        assert!(error.contains("aq4-gemm-wmma"));
        probe_fault_checkpoint(18, "aq4-gemm-wmma").unwrap();
    }

    #[test]
    fn aq4_ragged_m_wmma_m127_probe_binding_matches_small_row_geometry_and_packed_layout() {
        let binding = aq4_wmma_probe_binding(127);
        assert_eq!(binding.scale_count, 2);
        assert_eq!(binding.group_size, 16);
        assert_eq!(binding.tensor_scale_bits, 1.0_f32.to_bits());
        ...
        assert_eq!(binding.rows, 32);
        assert_eq!(binding.cols, 4_096);
        assert_eq!(binding.batch_count, 127);
        assert_eq!(binding.rows / 16, 2, "WMMA grid.x");
        assert_eq!(binding.matrix_elements().unwrap(), 131_072);
        assert_eq!(binding.packed_index_bytes().unwrap(), 65_536);
        assert_eq!(binding.scale_index_bytes().unwrap(), 8_192);
        assert_eq!(binding.input_elements().unwrap(), 520_192);
        assert_eq!(binding.output_elements().unwrap(), 4_064);
        assert_eq!(binding.packed_index_bytes().unwrap() % 16, 0);
        assert_eq!(binding.cols * std::mem::size_of::<f32>() % 16, 0);
        let scale_indices = aq4_probe_scale_indices(binding).unwrap();
        assert_eq!(scale_indices.len(), 8_192);
        ...
        force_probe_failure(ProbeFaultStage::Aq4GemmWmmaRaggedM);
        let error = probe_fault_checkpoint(21, "aq4-gemm-wmma-ragged-m").unwrap_err();
        assert!(error.contains("aq4-gemm-wmma-ragged-m"));
        probe_fault_checkpoint(21, "aq4-gemm-wmma-ragged-m").unwrap();
    }

    #[test]
    fn aq4_group8_wmma_m128_probe_binding_matches_target_geometry_and_packed_layout() {
        let binding = aq4_wmma_group8_probe_binding(128);
        assert_eq!(binding.scale_count, 2);
        assert_eq!(binding.group_size, 8);
        assert_eq!(binding.tensor_scale_bits, 1.0_f32.to_bits());
        assert_eq!(binding.row_scale_count, 0);
        assert_eq!(binding.rows, 1_024);
        assert_eq!(binding.cols, 4_096);
        assert_eq!(binding.batch_count, 128);
        assert_eq!(binding.rows / 16, 64, "WMMA grid.x");
        assert_eq!(binding.batch_count / 128, 1, "WMMA grid.y");
        assert_eq!(binding.matrix_elements().unwrap(), 4_194_304);
        assert_eq!(binding.packed_index_bytes().unwrap(), 2_097_152);
        assert_eq!(binding.scale_index_bytes().unwrap(), 524_288);
        assert_eq!(binding.input_elements().unwrap(), 524_288);
        assert_eq!(binding.output_elements().unwrap(), 131_072);
        assert_eq!(binding.packed_index_bytes().unwrap() % 16, 0);
        assert_eq!(binding.cols * std::mem::size_of::<f32>() % 16, 0);
        let scale_indices = aq4_probe_scale_indices(binding).unwrap();
        assert_eq!(scale_indices.len(), 524_288);
        assert!(
            scale_indices
                .iter()
                .all(|&index| usize::from(index) < binding.scale_count)
        ...
        force_probe_failure(ProbeFaultStage::Aq4GemmWmmaGroup8);
        let error = probe_fault_checkpoint(20, "aq4-gemm-wmma-group8").unwrap_err();
        assert!(error.contains("aq4-gemm-wmma-group8"));
        probe_fault_checkpoint(20, "aq4-gemm-wmma-group8").unwrap();
    }
