...
impl PackageAq4ResidentMatvec {
    ...
    /// Direct-only gfx1201 M=1 wave-shuffle experiment. This stays outside the projection
    /// dispatcher so it can be compared with the production lm_head path without changing it.
    pub fn matvec_shuffle_prototype(
        &self,
        input_buffer: &ullm_runtime_sys::RuntimeBuffer,
        output_buffer: &mut ullm_runtime_sys::RuntimeBuffer,
        stream: &mut ullm_runtime_sys::RuntimeStream,
        ...
        let aq4 = self.aq4_storage(label)?;
        ullm_runtime_sys::aq4_matvec_shuffle_prototype_f32(
            aq4.index_buffer,
            aq4.scale_buffer,
            aq4.codebook_buffer,
            aq4.scale_values_buffer,
        ...
        .map_err(|err| format!("failed to run {label} AQ4 matvec shuffle prototype: {err}"))
    }
        ...
        match &self.storage {
            PackageResidentMatvecStorage::Aq4 { .. } => {
                ...
                let result = match executable {
                    ...
                    ExecutableOperation::HipAq4GemmRegisterBm8Group8F32 => started
                    ...
                    ExecutableOperation::HipAq4GemmWmmaGroup8RaggedMF32 => started
                        .execute_aq4_gemm_wmma_group8_ragged_m_f32(
                            aq4.index_buffer, aq4.scale_buffer, aq4.codebook_buffer,
                            aq4.scale_values_buffer, input_buffer, aq4.row_scale_buffer,
    ...
    /// Direct-only gfx1201 AQ4 matvec-add wide-load experiment. This is intentionally not wired
    /// into resident projection dispatch and is used only by isolated differential/timing runs.
    pub fn matvec_add_wide_load_prototype(
        &self,
        input_buffer: &ullm_runtime_sys::RuntimeBuffer,
        residual_buffer: &ullm_runtime_sys::RuntimeBuffer,
        output_buffer: &mut ullm_runtime_sys::RuntimeBuffer,
        ...
        let aq4 = self.aq4_storage(label)?;
        ullm_runtime_sys::aq4_matvec_add_wide_load_prototype_f32(
            aq4.index_buffer,
            aq4.scale_buffer,
            aq4.codebook_buffer,
            aq4.scale_values_buffer,
        ...
        .map_err(|err| format!("failed to run {label} AQ4 matvec add wide-load prototype: {err}"))
    }
