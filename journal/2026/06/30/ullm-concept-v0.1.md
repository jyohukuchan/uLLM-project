...
- Firecrawl の検索とスクレイピングは利用可能だった。
- 検索フィードバックは、この環境では `DB_DISABLED` で送信できなかった。

## 今回の変更点

- 初期構想を、量子化フォーマット単体ではなく、量子化器、実行形式、バックエンド、スケジューラ、ベンチマークを含むプロジェクト構想に整理した。
- `aq` と `sq` を、単なる処理経路ではなく、保存形式、量子化方針、実行制約が異なる二系列として分離した。
- Qwen3 系以降、MTP、DiffusionGemma、prefill/decode 分離を、後付けではなく最初から拡張点として扱う方針にした。
