...
- opt-in時だけ`kernel-stages.jsonl`（固定座標のsummary）と`kernel-stages.f32le`（Phase 1 CPU `StageEmitter`と同じframed f32le protocol）を出す。各rowは最大16 MiBで、terminal frameを必須にする。manifestと`SHA256SUMS`にsidecarのschema・stage順・byte数・完全なguard setとそのSHA-256 identityを記録する。
