...
- `ae543a9` で metrics lane が source/active の model ID、upstream revision、tokenizer aggregateを再検証する。source exporter の `--threads` は PyTorch intra/inter-op threadsだけを制御し、plan は再現性のため1固定とする。8/16/32はCPU時間の候補だが、BF16 reduction順序の最下位bit/top-k tieが変わり得るため、個別artifactの全SHAとlegacy cross-checkを再検証する。
