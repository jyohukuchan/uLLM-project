...
- launcherを`0994367b`、harnessを`ffbb9cc3`へ固定し、正式probe 1回のprocess countをDocker 9、docker exec 6、互換container curl total 6、version 1、非version endpoint区分5へ固定した。9本の実argv fixtureと旧total 5の拒否を維持した。
...
- AMD-SMI processのowner zeroはexact `No running processes detected` sentinelだけを受理し、active ownerは保存済み実schemaのknown fieldsと正のinteger PIDを要求する共通strict parserへ統合した。sentinel混在、別文字列、extra fields、bool PID、他GPUを拒否し、raw本文の代わりにSHA-256とsecret-free reason/type/keysを保存する。
- base/profile readyと両canonical dry-runを再生成した。主要204、marker 55（25 subtests）、diagnostic capture 11、capture関連85 testsが通過した。dry-runではpoll/keepaliveを含む全process countが0である。
