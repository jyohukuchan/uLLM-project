...
- CPU fallback、HIP staging fallback、HIPRTC JIT materialize kernel経路に加えて、materialize済みf32 matrixからf32 matvecへつなぐ最小kernel境界、RMSNorm境界、SiLU-mul境界、Sigmoid-mul境界、f32 add境界、runtime RoPE境界、runtime causal attention境界、runtime decode attention境界、runtime paged decode attention境界、paged decode state/step、Qwen3 self attention runtime shape、Qwen3 self attention q projection split、Qwen3 self attention runtime weights、Qwen3 self attention prepared sequence、Qwen3 self-attn block step state、Qwen3 decoder layer step state、Qwen3 post attention runtime weights、Qwen3 decoder layer runtime weights、Qwen3 decoder layer runtime、depthwise conv1d境界、linear attention gate/beta境界、linear attention recurrent境界、実packageのlinear attention/self-attention/MLP部分workflow smokeまで通った。Qwen3.5 self-attn MLP block smokeではpaged decode step出力をlayer-level partial decoder経路へ渡せるようになった。
