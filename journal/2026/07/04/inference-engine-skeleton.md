...
- `Qwen3DecoderLayerSequenceOutput` がattention/projection中間値を返せるようになったので、次は `package-self-attn-mlp-block-smoke` のself-attn block sequenceとdecoder layer sequenceの二重実行を1回のlayer sequence実行へ寄せる。
