...
- full-model driver v2をraw-v2へ取り込むrole-aware adapterを追加した。driver/served workerを別identityとして検証し、2 warmup + 10 measuredの各artifactをimmutable保存してmeasurement/state/driver-lifecycle inputへ写像する。driver lifecycle inputはP1 traceではなく、trace linkはnullのままpromotion不可とした。role/case/raw swap、partial/reuse、duplicate/unknown、reset/fallback/preflight failureを負例化した。またbinder/result builder/final validatorはplanning manifestを固定expanderで再展開し、canonical hashまで再計算した縮小matrixを拒否する。
- production binderは公式stage contract（84/2,245/3,885、合計6,214）とcanonical case-set digestもpath非依存で要求する。custom縮小manifestは通常bindを拒否し、`--fixture-only`の場合だけfixture-onlyかつpromotion不可のidentityとして隔離する。final validatorもfixture identityをproduction evidenceとして受理しない。
