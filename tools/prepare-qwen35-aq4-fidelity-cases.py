...
def validate_plan(plan: dict[str, Any], *, expected_split_sha: str, expected_policy_sha: str, expected_calibration_sha: str, expected_served_sha: str, expected_package_sha: str, expected_worker_sha: str, expected_guard_sha: str, expected_device_architecture: str, expected_quantized_revision: str) -> None:
    ...
    if source.get("threads") != 16 or not isinstance(source.get("command_template"), str) or "--threads 16" not in source["command_template"] or any(flag not in source["command_template"] for flag in ("--split-root SPLIT_ROOT", "--expected-split-manifest-sha256 EXPECTED_SPLIT_MANIFEST_SHA256", "--expected-policy-sha256 EXPECTED_POLICY_SHA256", "--expected-calibration-cases-sha256 EXPECTED_CALIBRATION_CASES_SHA256", "--expected-cases-sha256 EXPECTED_SOURCE_CASES_SHA256")) or not isinstance(active.get("command_template"), str) or any(flag not in active["command_template"] for flag in ("--expected-split-manifest-sha256", "--expected-policy-sha256", "--expected-calibration-cases-sha256", "--expected-served-model-manifest-sha256", "--expected-package-manifest-sha256", "--expected-worker-binary-sha256", "--expected-guard-sha256", "--expected-device-architecture", "--expected-quantized-artifact-revision")):
