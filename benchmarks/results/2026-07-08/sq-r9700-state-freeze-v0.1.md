...
| T1 package-backed prefill component runner | done for v1 smoke | `tools/run-package-prefill-component-workload.py` runs package-backed component smokes and emits `inference-benchmark-result-v0.1` rows; R9700 AQ4 k-proj smoke verified `batching.mode=real` and `prefill_real_batch=true`; guarded by `phase-t1-package-prefill-component-runner-v1.md`. |
| T1 full package total throughput runner | not done | Request-batch prefill, decode, and end-to-end total throughput are still needed before SQ throughput comparison. |
...
9. Extend T1 from package-backed component rows to full package request-batch prefill/decode/end-to-end rows before using total throughput rows for SQ performance decisions.
