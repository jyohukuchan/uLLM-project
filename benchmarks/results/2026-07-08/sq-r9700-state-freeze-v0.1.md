...
2. Keep `kup6_gate5_down5` as a current six-layer regression subset, but do not promote it as the SQ quality policy because the six-layer token-id model-loop prompt bundle fails strict top1 on `len4` and `case_a`.
...
12. Move to T5 AQ4/FP8 throughput comparison after the T1 full package total-throughput runner exists and the T2 prompt-bundle subset is encoded in the runtime path.
13. Run vLLM comparison only after uLLM R9700 `batch=1/4/8` AQ4 and FP8 rows share the same schema.
