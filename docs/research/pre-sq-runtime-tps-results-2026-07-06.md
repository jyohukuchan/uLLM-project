...
## Text-Prompt TPS and Output Probe

The synthetic `len:16` token sequence is useful for stable performance comparisons, but it is too
weak for output-quality observation. `tools/run-package-token-prompt-bench.py` now wraps the
existing token-ID CLI with a local Hugging Face tokenizer:

- encode text prompt to token IDs without adding tokenizer support to the Rust runtime;
- call `package-token-ids-bench` with the same optimized HIP-kernel requirements used above;
...
- Text-prompt decode preserves the `~18-20 tok/s` range after the paged-attention fix. The
