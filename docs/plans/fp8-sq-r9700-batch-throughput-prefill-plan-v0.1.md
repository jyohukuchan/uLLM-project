...
- prompt-suite guardはv0.3へ進み、`generated_token_ids`、`decoded_text.generated`、`decoded_text.generated_without_stop_sequence`、top logitsの完全一致をstrict診断として保存しつつ、SQ候補を前進させる採用gateは `acceptance_mode=behavioral` に分離した。

## 次の行動

1. T2 short guardのstrict top1一致は境界調査用の診断として残す。SQ候補を次へ進めるprompt-suite gateでは `acceptance_mode=behavioral` を正式採用し、exact token/text/logit一致はblockerではなくdrift診断として扱う。
...
7. top-k overlap、AQ4 top1 rank、logit gapは診断指標として保存するが、それだけでSQ候補前進を止めない。出力が壊れる場合はbehavioral prompt-suiteで止める。
...
## 2026-07-09 progress: T2 SQ FP8 direct matvec pair/triple

前回の要点:

- SQ FP8 direct pathは単発 `matvec` と `matvec_batch` まで進んだ。
- ただしself-attention Q/K/V projectionでは、SQ tensorが複数ある場合でも `matvec_pair_with` / `matvec_triple_with` が単発matvecの連打へ落ちる状態だった。
- AQ4 fused pathと同じ呼び出し粒度に近づけるには、少なくともpair/triple projection境界をSQ FP8 direct pathへ広げる必要があった。
...
- CPU unit testと、`ULLM_REQUIRE_HIP_SQ_FP8_MATVEC_PAIR_KERNEL=1` / `ULLM_REQUIRE_HIP_SQ_FP8_MATVEC_TRIPLE_KERNEL=1` 付きHIP unit testでmixed scale granularityの出力を確認した。

判断:

- SQ FP8は単発、batch、pair、triple matvec API境界でF32 materializeなしに実行できるようになった。
- self-attention Q/K/V projectionに複数SQ tensorを入れる候補を作れば、単発kernel連打ではなくSQ FP8 triple境界を踏める。
...
## 2026-07-09 progress: T2 SQ FP8 q16/k16/v16 short batch guard
...
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-q16-k16-v16-short-batch-guard-v1.md` に保存した。
...
## 2026-07-09 progress: T2 SQ FP8 qkv layers3/7 prompt bundle
...
- SQ側は `ULLM_REQUIRE_HIP_SQ_FP8_MATVEC_TRIPLE_KERNEL=1` でtriple direct kernelを必須化した。
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-layers3-7-q16-k16-v16-prompt-bundle-v1.md` に保存した。

実測値:

| row | FP8 tensors | prefill tok/s | decode tok/s | end-to-end tok/s | VRAM consumed bytes | final top1 |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| AQ4 baseline | 0 | 66.811351 | 80.883171 | 35.370726 | 4443144192 | `24218,4105,329` |
| SQ `layers3/7 q16/k16/v16` | 6 | 62.866188 | 77.707480 | 34.095768 | 4499853312 | `24218,4105,329` |
...
## 2026-07-09 progress: T2 SQ FP8 qkv layers3/7/11 prompt bundle
...
- SQ側は `ULLM_REQUIRE_HIP_SQ_FP8_MATVEC_TRIPLE_KERNEL=1` でtriple direct kernelを必須化した。
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-layers3-7-11-q16-k16-v16-prompt-bundle-v1.md` に保存した。
...
## 2026-07-09 progress: T2 SQ FP8 qkv layers3/7/11/15 prompt bundle
...
- SQ側は `ULLM_REQUIRE_HIP_SQ_FP8_MATVEC_TRIPLE_KERNEL=1` でtriple direct kernelを必須化した。
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-layers3-7-11-15-q16-k16-v16-prompt-bundle-v1.md` に保存した。
...
- 失敗は `case_a` のみで、AQ4 top1 `4105` はSQ top8内rank 2に残る。
- SQ top1 `5582` とAQ4 top1 `4105` のlogit差は `0.001344681` と小さいが、strict top1 policyでは失敗なのでpromoteしない。
...
## 2026-07-09 progress: T2 SQ FP8 qkv layer15 split prompt bundle
...
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-layer15-split-v1.md` に保存した。
...
## 2026-07-09 progress: T2 SQ FP8 qkv layer15 pair prompt bundle
...
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-layer15-pair-v1.md` に保存した。
...
## 2026-07-09 progress: T2 SQ FP8 qkv layer19 q8 full QKV prompt bundle
...
- SQ側は `ULLM_REQUIRE_HIP_SQ_FP8_MATVEC_TRIPLE_KERNEL=1` を使い、triple direct kernelを必須化した。
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-layer19-q8-full-qkv-v1.md` に保存した。
...
## 2026-07-09 progress: T2 SQ FP8 qkv layer19 q8 full QKV short batch guard
...
- AQ4 baselineは `phase-t2-sq-fp8-qkv-q16-k16-v16-short-batch-guard-v1` の同一workload結果を再利用した。
- SQ側は `ULLM_REQUIRE_HIP_SQ_FP8_MATVEC_TRIPLE_KERNEL=1` でtriple direct kernelを必須化した。
- 結果は `benchmarks/results/2026-07-09/package-batch-throughput/phase-t2-sq-fp8-qkv-layer19-q8-full-qkv-short-batch-guard-v1.md` に保存した。
...
- prompt-suiteでは、token ID一致だけでなくdecoded text一致も正式な診断指標として扱える。
- ただし、exact decoded text一致をSQ FP8採用の必須条件にすると過剰に保守的になる。strict一致は回帰検出とdrift把握のために残し、採用前進のgateは後続のbehavioral modeへ移す。
- short token-id model-loop guardは引き続きstrict top1で候補境界を調査するが、full SQ policyへ進めるかどうかはbehavioral prompt-suite、throughput、memoryで判断する。

次の行動:

1. layer23 `k16` passing branchまたは次のSQ candidateを、v0.2 prompt-suite guardへ接続する。
2. behavioral prompt-suiteを通したcandidateだけをbatch throughput / memory comparisonへ進める。
...
- `tools/compare-package-token-prompt-suite.py` に `--acceptance-mode strict|behavioral` を追加した。`behavioral` ではexact token/text/logit一致を診断として保存するが、採用gateはprompt一致、candidate verified、非空生成、output health acceptedに限定する。
...
- top-k logitsはrank driftを起こしており、strict guardとしては不合格である。
- ただしbehavioral gateでは合格であり、SQ FP8候補探索ではexact token/text/logit一致をblockerにしない。
- この結果はSQ FP8を次のbatch throughput / memory comparisonへ進めるための接続smokeとして扱う。
