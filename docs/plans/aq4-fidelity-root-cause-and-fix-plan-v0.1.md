...
Status: Phase 1・Phase 2・Phase 2b完了。H8(深さ方向蓄積)単独では最終相対L2 0.615を説明できないと判定。次はPhase 3b(構成差監査、CPU-only)。P2 fidelity calibrationはNo-Go凍結中、GPU実行は未承認。
...
3. Decision Treeに従い、**Phase 3b（診断harnessと実productionの構成差監査、H6、CPU-only）**を次に実施する。
4. Phase 3bで説明が尽きない場合のみPhase 3c（AQ4 CPU vs AQ4 GPU kernel差分、単発承認GPU window）を申請する。GPU windowはCPU-onlyの調査（Phase 2〜3b）で候補を絞り込んだ後にのみ申請する。
