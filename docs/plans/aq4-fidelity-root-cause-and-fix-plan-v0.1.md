...
凍結policy(`docs/proposals/aq4-p2-fidelity-holdout-protocol-v0.1.md`)の8指標中7指標が余裕を持って合格した。

| 指標 | 観測値 | 必要閾値 | 判定 |
|---|---:|---:|---|
| logits_relative_l2 | 0.0870 | ≤0.1468 | ✅ |
| logits_cosine | 0.9965 | ≥0.9856 | ✅ |
| hidden_relative_l2 | 0.1345 | ≤0.1916 | ✅ |
| hidden_cosine | 0.9910 | ≥0.9800 | ✅ |
| topk_overlap_rate_k10 | 0.8542 | ≥0.7900 | ✅ |
| bf16_top1_retained_in_aq4_top10_rate | 100%(24/24) | ≥89.9%(Wilson下限) | ✅ |
| hidden_max_abs | (診断専用、閾値なし) | — | ✅ |
| **token_agreement_rate** | 83.3%(20/24) | ≥89.9%(Wilson下限は67.6%) | **❌** |

**24ケース全件で「病的」な相対L2 > 1.0の破局的失敗はゼロ**(07/15は19/19全件が>1.0で失敗)。しかし4/24ケースでgreedy top-1が僅差で入れ替わっており、これが唯一の不合格指標(token_agreement_rate)の原因になっている。

加えて、`docs/specs/aq4-p2-calibration-evidence-binding-v0.1.md`と`docs/proposals/aq4-p2-fidelity-holdout-protocol-v0.1.md`のあいだに、指標定義そのものの既存の文書矛盾（後者はWilson token-agreement・logits max-abs bound無し・hidden max-abs診断専用扱いだが、前者はgreedy mismatch行数=0要求・logits max-abs boundあり）が判明した。この矛盾はterraが独自解決せず明示的にfail-closedで分離し、`formal_p2_status=blocked_contract_resolution`として記録した。

**現状のP2昇格判定は「no-go」のままである。** 理由は2つに分離される: (1) token_agreement_rateがWilson下限で閾値未達、(2) 凍結policy文書間の矛盾が未解決。前者は根本原因のさらなる追跡(残存する僅少な数値誤差)またはholdoutサンプル数の拡大、後者はdocs/proposals/aq4-p2-correctness-fidelity-amendment-v0.1.mdの正式承認手続きが必要。
...
## Phase 7: 正式P2 fidelity gate再実行と計画へのハンドバック — 完了、no-go(部分改善)

Status: **完了**（実行: 親エージェント、tooling準備: `gpt-5.6-terra`/`max`、journal: [aq4-phase7-p2-fidelity-preparation-v0.1.md](../../journal/2026/07/17/aq4-phase7-p2-fidelity-preparation-v0.1.md)）

### 結果

新規独立holdout48件(07/15の凍結holdoutとは別、tuning/diagnosticと非重複を機械確認済み)で正式calibrationを実行した。8指標中7指標が合格。**token_agreement_rate(Wilson下限0.676 < 必要0.899)だけが不合格。** 病的な相対L2>1.0の破局的失敗は24ケース中0件(07/15は19/19)。
...
2. ~~全ケースが相対L2 ceiling 1.0を含む5指標policyに合格することを確認する。~~ **8指標中7指標合格、token_agreement_rateのみ不合格。**
3. 既存のAQ4 release gate（stop/soak/cancel/failure-restart）と`same_artifact_all_m1` behavioral oracleに回帰がないことを確認する。**未実施**(No-Goのため、この確認はGo判定後に行う位置づけ)。
4. Go判定が得られたら、`aq4-production-prefill-decode-optimization-plan-v0.1.md`のP2へ結果をハンドバックし、P3以降の性能最適化候補選抜を再開可能にする。**Go未達のため未実施。**
5. `docs/proposals/aq4-p2-correctness-fidelity-amendment-v0.1.md`の承認要否を判断し、必要なら正式binding specへ反映する。**判明した文書矛盾を含め、ユーザー判断待ち。**

### Exit Criteria

- P2 fidelity gateがGoで確定している。 ❌ 未達(no-go、token_agreement_rate1指標のみ不合格)。
- 親計画（性能最適化計画）が本計画の完了を前提に再開できる状態になっている。 ❌ 未達。
