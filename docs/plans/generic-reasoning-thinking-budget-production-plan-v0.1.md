...
### 13.5 llama.cpp比較baseline

llama.cppはuLLMと同時にOpenWebUIへ登録された外部参考providerである。
...
`/run/ullm/r9700.lock`を共有しない。同時生成は性能結果を汚すため、比較requestは必ず交互に実行する。
...
異なるため、llama.cppはAPI、reasoning表示、速度傾向の参考に限る。
...
には8000だけのruleが記録され、firewall unitは監査時active (exited)だがdisabledだった。live nftablesは
権限不足で独立確認できていない。8001を隔離済みと断定せず、deploymentまたはbenchmark前にroot権限で
`/etc/ullm/ullm-openai.nft`、live table、repository定義を照合する。
