...
- 出力は新しいv1 schema/rootだけへatomic publishし、既存 `source-oracle-v1/v2` を上書きしない。
...
`tools/validate-qwen35-aq4-p2-full-calibration.py` はunknown field、duplicate key、non-finite JSON、symlink、TOCTOU、sidecar offset/length/hash、stable top-k順序、legacy cross-checkを再計算する。nonfinite vector rowを含むartifactは`blocked`として扱う。
...
どちらもreference/candidateの全rowをlockstepでchunk読み出しし、f64累積で次を計算する。
