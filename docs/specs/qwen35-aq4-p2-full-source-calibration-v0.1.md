...
この文書が扱う`tools/export-qwen35-aq4-source-calibration.py`は既存のBF16 calibration exporterである。今回のproduction用`tools/capture-aq4-p2-production-source-oracle.py`は同じfull-vector sidecarの意味論を使うが、CPU実行dtypeを`float32`としてmanifestの`runtime.dtype`へ実値で記録する。checkpointの保存dtypeとruntime dtypeを混同してはならず、vector sidecarの`dtype`は両経路とも`f32` little-endianである。
...
1. `source_gate`: independent sourceとAQ4 target sidecarを比較する。比較対象のruntime dtypeは各source manifestの実値を使い、この既存exporterはBF16、現行production captureはF32である。
