...
当面は `.ullm` を directory container 名として使い、`model.ullm/` の中に manifest、tensor、codebook、scale などを置く。単一ファイル化するかどうか、単一ファイル化する場合の内部構造は後で決める。
...
`tensors/`、`codebooks/`、`scales/` は、現時点では固定された量子化仕様の一部ではなく、manifest が解釈方法を指定する論理ストレージである。

- `tensors/`: FP/BF baseline weight、packed index、hardware-native payload など、実際の tensor payload を置く。
- `codebooks/`: LUT/codebook 系 candidate が必要とする table を置く。使わない candidate では空または存在しなくてよい。
- `scales/`: per-tensor、per-channel、per-group、block scale などの scale payload を置く。使わない candidate では空または存在しなくてよい。

どのファイルをどう読むかは、固定ルールではなく manifest の `payload_schema`、`quant_experiment_id`、tensor table、artifact table が決める。
...
1. Qwen3 実装前に、既存推論エンジンの token/s をコンテキスト長、batch、TP、PP、GPU 数などの条件別に測定する。
2. v0.1 では `.ullm` directory container と manifest schema を決める。ただし `aq` と `sq` の詳細仕様は固定せず、実験 payload を記録できる余地を残す。
3. `aq` と `sq` は、実験 protocol、結果 schema、採用判定基準を先に決める。具体的な量子化方式は測定しながら決める。
4. Qwen3-14B と Qwen3-30B-A3B を初期 correctness target にする。
5. HIP on V620/R9700、MI300X、AVX-512 の順で backend を進める。
6. その段階で一度発表・宣伝できる benchmark と demo を作る。
7. Qwen3.5 または Gemma4 を早期の新技術検証 target に追加する。
8. JAX/TPU、NPU、Ascend、NVIDIA backend は順次 plugin/backend として対応する。
